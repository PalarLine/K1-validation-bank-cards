...
result.map(function(item) {
  item = item.split(' ').join('').split('-').join('')
    let arrOfCardNumbers = [...item];
       
        if (luhnAlgorithm (arrOfCardNumbers) == true) {
          
           let numberOfValidCards = arrOfCardNumbers.reverse().join('');
                      
                console.log(numberOfValidCards.substring(0, 4) + ' ' + numberOfValidCards.substring(4, 8) + ' ' + numberOfValidCards.substring(8, 12) + ' ' + numberOfValidCards.substring(12, 16));  
